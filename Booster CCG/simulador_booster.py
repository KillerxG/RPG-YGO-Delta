...
from PyQt6.QtWidgets import (
    ...
    QGraphicsDropShadowEffect, QGraphicsOpacityEffect  # <-- recolocados
)
from PyQt6.QtGui import QPalette, QColor, QIcon, QPixmap, QFont
from PyQt6.QtCore import (
    Qt, QSize, QEasingCurve, QPropertyAnimation, QParallelAnimationGroup,
...
class ResultPage(QWidget):
    def __init__(self, go_back_to_boosters):
        ...
        self.speed = 'slow'   # padrão
        self.sr_glow = True   # glow ativo nas Super Rare

        self._last_booster = None
        ...
        # toolbar simplificada — apenas "Abrir de novo"
        bar = QHBoxLayout()
        bar.setContentsMargins(8, 4, 8, 4)
        bar.setSpacing(10)

        self.btn_again = StyledButton("Abrir de novo")
        self.btn_again.setFixedHeight(44)
        self.btn_again.clicked.connect(self.open_again)

        bar.addStretch(1)
        bar.addWidget(self.btn_again)
        bar.addStretch(1)
    ...
    def show_results(self, booster_name: str, super_cards, secret_card):
        self._last_booster = booster_name

        # parar/limpar animações antigas
        ...
        pause_step, dur_super, dur_secret = self.timings()
        glow_color_sr = "#cccccc" if self.sr_glow else None

        # Super Rares
        for idx, p in enumerate(super_cards):
            container, op_effect = self._card_widget(p, glow_color=glow_color_sr, rarity_text="Super Rare")
    ...
    def _pick_cards(self, booster_name: str):
        script_dir = Path(__file__).resolve().parent
        cards_dir = script_dir / "cards"
        super_dir = cards_dir / booster_name / "super raras"
        secret_dir = cards_dir / booster_name / "secretas raras"
        super_cards = list_images(super_dir)
        secret_cards = list_images(secret_dir)
        if len(super_cards) < 4 or len(secret_cards) < 1:
            raise RuntimeError(f"Pastas insuficientes:\n{super_dir}\n{secret_dir}")
        chosen_super = random.sample(super_cards, 4)
        chosen_secret = random.choice(secret_cards)
        return chosen_super, chosen_secret

    def open_again(self):
        # parar animações ativas antes de refazer
        if self._anim_group and self._anim_group.state() != QAbstractAnimation.State.Stopped:
            try:
                self._anim_group.stop()
            except Exception:
                pass

        if not self._last_booster:
            return
        try:
            s, c = self._pick_cards(self._last_booster)
            self.show_results(self._last_booster, s, c)
        except Exception as e:
            QMessageBox.warning(self, "Falha ao abrir de novo", str(e))
